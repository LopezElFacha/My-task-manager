...
function skip(to) {
  ...
  if (!isRandomed) {
    ...
    if (to == 'prev') {
      stop()
      currentSong =
        ramdomizedPlaylist[ramdomizedPlaylist.indexOf(currentSong) - 1]
      if (currentSong == undefined)
        currentSong = ramdomizedPlaylist[ramdomizedPlaylist.length - 1]
      console.log(currentSong, ramdomizedPlaylist)
      playpause()
    } else if (to == 'next') {
      if (
        ramdomizedPlaylist.indexOf(currentSong) + 1 >=
        ramdomizedPlaylist.length
      ) {
        randomed()
      } else {
        stop()
...
function mute() {
  if (song.muted) {
    ...
    document.getElementById('mute').className =
      'fa fa-volume-up btn-reproductor'
  } else {
    song.muted = true
    document.getElementById('mute').className =
      'fa fa-volume-off btn-reproductor'
  }
}
...
song.addEventListener('error', function () {
  stop()
  if (confirm('Hubo un error. ¿Recargar?'))
